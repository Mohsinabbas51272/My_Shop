...
import { Plus, Loader2, PackageX, ShoppingBag, Clock, CheckCircle2, Receipt, Trash2, Edit2, MessageCircle, AlertCircle } from 'lucide-react';
...
export default function Dashboard() {
    ...
    const editOrderMutation = useMutation({
        ...
        onError: (err: any) => alert(err.response?.data?.message || 'Failed to update order'),
    });
                ...
                {activeTab === 'shop' ? (
                    ...
                    <div className="space-y-6">
                        {complaintsLoading ? (
                            <div className="flex flex-col items-center justify-center py-24">
                                <Loader2 className="w-12 h-12 text-[var(--primary)] animate-spin" />
                                <p className="mt-4 text-[var(--text-muted)] font-medium">Loading your queries...</p>
                            </div>
                            ...
                            <div className="flex flex-col items-center justify-center py-24 text-[var(--text-muted)] bg-[var(--bg-card)]/30 rounded-3xl border border-dashed border-[var(--border)]">
                                ...
                                <p className="text-xl font-medium">No queries submitted yet</p>
                                <button
                                    onClick={() => setSearchParams({ tab: 'shop' })}
                            ...
                            <div className="grid grid-cols-1 gap-6">
                                {complaints.map((complaint: any) => (
                                    <div key={complaint.id} className="bg-[var(--bg-card)] border border-[var(--border)] rounded-2xl p-4 md:p-6 hover:border-[var(--primary)]/30 transition-colors">
                                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4 pb-4 border-b border-[var(--border)]/50">
                                            <div className="grid grid-cols-2 gap-4">
                                                <div>
                                                    <label className="block text-[10px] font-bold uppercase tracking-widest text-[var(--text-muted)] mb-1">Query ID</label>
                                                    <p className="font-mono text-[var(--text-main)]">#{complaint.id}</p>
                                                </div>
                                                {complaint.orderId && (
                                                    <div>
                                                        <label className="block text-[10px] font-bold uppercase tracking-widest text-[var(--text-muted)] mb-1">Order ID</label>
                                                        <p className="font-mono text-[var(--primary)]">#{complaint.orderId}</p>
                                                    </div>
                                                )}
                                                <div>
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-sm text-[var(--text-muted)]">{new Date(complaint.createdAt).toLocaleDateString()}</p>
                                                </div>
                                            </div>
                                        </div>

                                        <div className="mb-4">
                                            <label className="block text-xs font-bold uppercase tracking-widest text-[var(--text-muted)] mb-2">Your Query</label>
                                            <p className="text-sm text-[var(--text-main)] bg-[var(--bg-input)]/30 p-3 rounded-lg border border-[var(--border)]">
                                                {complaint.subject}
                                            </p>
                                        </div>
                                        ...
                                        {complaint.description && (
                                            <div className="mb-4">
                                                <label className="block text-xs font-bold uppercase tracking-widest text-[var(--text-muted)] mb-2">Description</label>
                                                <p className="text-sm text-[var(--text-main)] bg-[var(--bg-input)]/30 p-3 rounded-lg border border-[var(--border)]">
                                                    {complaint.description}
                                                </p>
                                            </div>
                                        ...
                                        {complaint.adminResponse ? (
                                            <div className="bg-green-500/10 border border-green-500/20 rounded-lg p-4">
                                                <div className="flex items-center gap-2 mb-2">
                                                    ...
                                                    <label className="text-xs font-bold uppercase tracking-widest text-green-500">Admin Response</label>
                                                </div>
                                                <p className="text-sm text-[var(--text-main)] mb-2">{complaint.adminResponse}</p>
                                                {complaint.adminRespondedAt && (
                                                    <p className="text-[10px] text-green-500/70">
                                                        Responded on {new Date(complaint.adminRespondedAt).toLocaleString()}
                                                    </p>
                                                )}
                                            </div>
                                        ) : (
                                            <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-lg p-4 flex items-center gap-3">
                                                ...
                                                <div>
                                                    <p className="text-sm font-semibold text-yellow-500">Awaiting Admin Response</p>
                                                    <p className="text-[10px] text-yellow-500/70">We'll respond to your query soon.</p>
                                                </div>
                                            </div>
                                        )}
                                    </div>
